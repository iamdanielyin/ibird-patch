            ...
            try {
                ...
                if (!omit) {
                    for (let key in dest) {
                        ...
                        const keySrc = path.resolve(process.cwd(), src, key);
                        fs.accessSync(keySrc, fs.constants.R_OK);
                        fsx.copySync(keySrc, dest[key]);
                    }
                } else {
                    //2.如果是对象配置，则再做处理
                    ...
                    for (const item of files) {
                        ...
                        const _src = path.resolve(process.cwd(), src, item);
                        const _dest = path.resolve(process.cwd(), dest, item);
